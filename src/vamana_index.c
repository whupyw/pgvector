...
void generate_random_neighbors(uint32_t num_points, uint32_t R, uint32_t *neighbors)
{
    if (R >= num_points)
    {
        printf("错误：邻居数 R 必须小于总点数 num_points。\n");
        return;
    }

    int *candidate_indices = (uint32_t *)ppalloc((num_points - 1) * sizeof(uint32_t));
...
#pragma omp parallel for
    for (int i = 0; i < num_points; i++)
    {
        ...
        for (int j = 0; j < num_points; j++)
        {
            if (j != i)
            {
                candidate_indices[idx++] = j;
            }
        }

        // Fisher-Yates 洗牌打乱候选列表
        for (int j = num_points - 2; j > 0; j--)
        {
            uint32_t rand_idx = rand() % (j + 1);
            swap(&candidate_indices[j], &candidate_indices[rand_idx]);
        }

        // 取前 R 个不同的邻居
        for (int j = 0; j < R; j++)
        {
            neighbors[i * R + j] = candidate_indices[j];
        }
    }

    pfree(candidate_indices);
}
...
float get_vec_from_compressed_data(const float *compressed_data, float *pivots_data, uint32_t location, float *vector, uint32_t num_pq_chunks, uint32_t dim)
{
    // 从压缩数据中获取向量
    // 计算这两个向量的距离
    // 返回距离
...
// 初始化候选池并开始搜索
void iterate_to_fixed_point(Scratch *scratch,float *pivots_data, uint32_t *compressed_vectors, uint32_t *neighbours, uint32_t LSize, uint32_t *init_id, float *target_vector)
{
    // 1.初始化操作
    // 记录已访问的节点
    // 存储当前最优的L个邻居
    ...
    uint32_t R = 10;
    // 候选集
    NeighborPriorityQueue L_nodes;
    init_queue(&L_nodes, LSize);
    uint32_t max_points = 1000;                                           // 最大节点数
    size_t num_bits = 100;                                                // 需要存储 100 位
    size_t num_ints = (num_bits + 31) / 32;                               // 计算需要多少 uint32_t
    uint32_t *bit_array = (uint32_t *)calloc(num_ints, sizeof(uint32_t)); // 创建一个 bit 数组并初始化为0
    ...
    if (init_id > max_points)
    {
        elog(ERROR, "init_id is out of range");
    }
    set_bit(bit_array, *init_id); // 将初始节点标记为已访问

    // 将初始节点加入候选集
    float distance = get_vec_from_compressed_data(compressed_vectors, pivots_data, *init_id, target_vector, 8, 128); // 计算距离
    Neighbor nn;
    nn.id = *init_id;
    ...
    // 已扩展的节点列表
    MyVector *expanded_nodes;
    vector_init(expanded_nodes);

    float *cur_node_neighbors = NULL;   // 估计大小
    float *neighbours_distances = NULL; // 估计大小

    MyVector *id_scratch;
    vector_init(id_scratch);
    MyVector *dist_scratch;
    ...
    while (has_unexpanded_node(&L_nodes))
    {
        ...
        float distance;
        // 从 compressed_vectors 中获取向量
        get_neighbors(cur_node.id, R, neighbours, cur_node_neighbors); // 获取邻居

        // 看看邻居是否在已访问列表中
        for (size_t i = 0; i < R; i++)
        {
            uint32_t neighbor_id = cur_node_neighbors[i];
            if (test_bit(bit_array, neighbor_id))
            {
                continue;
            }
            set_bit(bit_array, neighbor_id); // 标记为已访问

            // 计算距离
            distance = get_vec_from_compressed_data(compressed_vectors, pivots_data, neighbor_id, target_vector, 8, 128);
            // 存入距离列表
    ...
    pfree(cur_node_neighbors);
    pfree(neighbours_distances);
    // 申请内存用于保存邻居、候选池等
}
...
// 搜索节点 加入候选集
void search_for_point_and_prune(Scratch *scratch, float *pivots_data, uint32_t *compressed_vectors, uint32_t *neighbours, uint32_t location, uint32_t Lindex, uint32_t *pruned_list)
{
    uint32_t init_id;

    // 获取初始节点ID 如果没有冻结点，就是一个入口点
    ...
    // 执行固定点迭代
    iterate_to_fixed_point(scratch,pivots_data, compressed_vectors, neighbours, Lindex, &init_id, query);
...
void vamana_link(float *pivots_data, uint32_t *compressed_vectors, uint32_t *neighbours, uint32_t dim, size_t num_points, uint32_t R, uint32_t L, uint32_t num_threads)
{
    ...
    {
        Scratch *scratch;
        init_scratch(scratch, num_points, R, L);
        ...
        search_for_point_and_prune(scratch,pivots_data, compressed_vectors, neighbours, i, L, pruned_list);
