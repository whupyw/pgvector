                                      ...
                                      int k, Vector *target, uint32_t vector_num)
{
    ...
    uint32_t io_limit = 10;
    uint32_t num_ios = 0;
    uint32_t beam_width = 20;
    ...
    init_queue(retset, k);
    is_visited = (uint32_t *)calloc(vector_num, sizeof(uint32_t));
        ...
        for (uint32_t i = 0; i < frontier_nhoods->size; i++)
        {
            ...
            for (uint32_t m = 0; m < n_count; m++)
            {
                ...
                if (!test_bit(is_visited, id))
                {
                    // 如果没访问过
                    // 计算距离 应该是计算与查询向量的距离
                    float distance = get_distance_to_target_by_id(init_id, target, target->dim);
                    Neighbor nn;
                    nn.id = id;
